...
test("digital human pipeline uploads a portrait and completes", async ({ page }) => {
  await openPipelineFromCreate(page, "Digital Human");
  await createProject(page, uniqueName("digital-human-project"));

  await page.getByLabel("Portrait image").setInputFiles(fixturePath("portrait.png"));
  await page.getByLabel("Narration").fill("Present the weekly launch notes in a calm studio voice.");

  await page.getByRole("button", { name: "Generate Video" }).click();

  await waitForTaskProgress(page);
  await waitForVideoResult(page);
  await expect(page.getByRole("heading", { name: "Digital Human" })).toBeVisible();
});
