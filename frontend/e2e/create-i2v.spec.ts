...
test("image to video pipeline uploads an image and completes", async ({ page }) => {
  await openPipelineFromCreate(page, "Image → Video");
  await createProject(page, uniqueName("i2v-project"));

  await page.getByLabel("Source image").setInputFiles(fixturePath("source.png"));
  await page.getByLabel("Motion Prompt").fill("Add a gentle camera push-in with floating nebula particles.");
  await selectFirstOption(page, "Media workflow");

  await page.getByRole("button", { name: "Generate Video" }).click();

  await waitForTaskProgress(page);
  await waitForVideoResult(page);
  await expect(page.getByRole("heading", { name: "Image → Video" })).toBeVisible();
});
